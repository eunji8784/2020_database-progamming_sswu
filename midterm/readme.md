...
1. 영화 검색
      * 장르별 검색: 선택박스에서 찾아보고 싶은 영화의 장르를 선택하고 검색 버튼을 누르면 해당하는 장르의 영화들만 검색할 수 있다.
      * 영화 제목으로 검색: 찾아보고 싶은 영화의 제목을 직접 입력해서 검색할 수 있다.
   이때 선택박스에서 아무것도 선택하지 않고 영화 제목도 입력하지 않으면 테이블에 있는 모든 영화 정보를 출력한다.
2. 고객 대여 정보 : 고객의 이메일을 입력하면 해당 고객의 고객 아이디와 이름, 그리고 대여해간 DVD 정보와 대여 날짜 등을 검색할 수 있다.
3. 상점 정보 : 현재 DVD를 대여할 수 있는 상점의 주소와 지역 등을 검색할 수 있다.
